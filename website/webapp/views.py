...
from . import generation_gpt2_topic
...
def poem(topic, start_words):
    output_poems = []
    generated = torch.tensor(tokenizer.encode(start_words)).unsqueeze(0)
...
def index_(request):
    ...
    if request.method == 'POST':
        topic = request.POST.get('input1')
        starting = request.POST.get('input2')

        poems = poem(topic, starting)
